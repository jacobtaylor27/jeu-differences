...
import { ID_PREFIX } from '@app/constants/database';
...
export class BmpService {
    ...
    async getBmpById(bmpId: string, filepath: string): Promise<Bmp> {
        const fullpath: string = path.join(filepath, ID_PREFIX + bmpId + '.bmp');
    ...
    async addBmp(bpmToConvert: ArrayBuffer, filepath: string): Promise<void> {
        ...
        const fullpath = path.join(filepath, ID_PREFIX + bmpId + '.bmp');
    ...
    async deleteBmpById(bmpId: string, filepath: string): Promise<boolean> {
        await fs.promises.unlink(path.join(filepath, ID_PREFIX + bmpId + '.bmp'));
        return false;
    }
}
